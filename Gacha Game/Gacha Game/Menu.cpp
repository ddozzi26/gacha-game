...
void Menu::ShowGachaStats() {
    std::cout << "\n=== 가챠 기록 ===\n";
    std::cout << "전투기 바디 가챠 횟수: " << gacha.GetBodyCount() << "회\n";
    std::cout << "전용 무기 가챠 횟수: " << gacha.GetWeaponCount() << "회\n";
    std::cout << "쓸모 없는 3성: " << gacha.GetStar3() << "개\n";
    std::cout << "기분 좋은 4성: " << gacha.GetStar4() << "개\n";
    std::cout << "최고급 5성 전투기: " << gacha.GetStar5Plane() << "개\n";
    std::cout << "5성 무기: " << gacha.GetStar5Weapon() << "개\n";
    std::cout << "꽝 5성: " << gacha.GetStar5Fail() << "개\n";
}

void Menu::CheatMenu() {
    int delta;
    std::cout << "\n\n=== 치트키 모드 ===\n\n현재 플레이어의 쥬얼 갯수를 조정하세요: ";
    while (!(std::cin >> delta)) {
        std::cout << "잘못된 입력입니다. 숫자를 입력해주세요: ";
        std::cin.clear();
